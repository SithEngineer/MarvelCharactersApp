...
class MarvelImageView(context: Context?, attrs: AttributeSet?,
    defStyleAttr: Int) : SimpleDraweeView(context, attrs, defStyleAttr) {
  ...
  private val characterMainColorPaint by lazy {
    Paint().apply {
      ...
      strokeWidth = DisplayUtils.convertDpToPixel(
          context, 5).toFloat()
  ...
  override fun onDraw(canvas: Canvas?) {
    if (canvas != null) {
      ...
      val leftHeightOffsetWithoutCharacterColorLine = DisplayUtils.convertDpToPixel(
          context, 40)
      val rightHeightOffsetWithoutCharacterColorLine = DisplayUtils.convertDpToPixel(
          context, 10)
      val leftHeightOffsetCharacterColorLine = DisplayUtils.convertDpToPixel(
          context, 40)
      val rightHeightOffsetCharacterColorLine = DisplayUtils.convertDpToPixel(
          context, 10)
